...
```{r remedy30}
m06_16a <- lm(gap_just ~ mujer + edad + educacion + movsubj + gap_perc ,data=subset(long01b, year==2016))
m06_17a <- lm(gap_just ~ mujer + edad + educacion + movsubj + gap_perc ,data=subset(long01b, year==2017))
m06_18a <- lm(gap_just ~ mujer + edad + educacion + movsubj + gap_perc ,data=subset(long01b, year==2018))
m06_19a <- lm(gap_just ~ mujer + edad + educacion + movsubj + gap_perc ,data=subset(long01b, year==2019))

m06_16 <- lm(gap_just ~ mujer + edad + educacion + mov_sub_c + gap_perc ,data=subset(long01b, year==2016))
m06_17 <- lm(gap_just ~ mujer + edad + educacion + mov_sub_c + gap_perc ,data=subset(long01b, year==2017))
m06_18 <- lm(gap_just ~ mujer + edad + educacion + mov_sub_c + gap_perc ,data=subset(long01b, year==2018))
m06_19 <- lm(gap_just ~ mujer + edad + educacion + mov_sub_c + gap_perc ,data=subset(long01b, year==2019))

m05a <- lme4::lmer(gap_perc ~ year + mujer + edad + educacion + movsubj + (1 | idencuesta),data=long01b)
m06a <- lme4::lmer(gap_just ~ year + mujer + edad + educacion + movsubj + gap_perc + (1 | idencuesta),data=long01b)
m05 <- lme4::lmer(gap_perc ~ year + mujer + edad + educacion + mov_sub_c + (1 | idencuesta),data=long01b)
m06 <- lme4::lmer(gap_just ~ year + mujer + edad + educacion + mov_sub_c + gap_perc + (1  | idencuesta),data=long01b)

texreg::screenreg(l = list(m06_16,m06_17,m06_18,m06_19,
                           m06_16a,m06_17a,m06_18a,m06_19a))

texreg::screenreg(l = list(m05a,m06a,m05,m06))
plot_model(m05,type = "pred",terms = "year")
plot_model(m06,type = "pred",terms = "year")
```

```{r remedy30}
# "educacion_encuestado (t1)" - "educacion_padre|madre (t1) (max_alcanzado)" 
# "isei_i (t1)" = b1*1 
# 
# id1 = moviseit1_t3 = (isei_t1 - isei_t3) # no hay variabilidad en el tiempo 
 id1 = (income_t1 - income_t4) # A

income_cwc <- income - mean(income) #between 
group_by(idencuestado) %>% mutate(income_cwc = income - mean(income)) #within
id1 = (income*b1 (idencuestado)) # B

# long01a$year <- as.numeric(long01a$year)
m16_con <- lm(lngap_just ~ mujer+edad+educacion+mov_subj_cwc+movsubj_mean ,data=subset(long01b, year==2016))
m17_con <- lm(lngap_just ~ mujer+edad+educacion+mov_subj_cwc+movsubj_mean ,data=subset(long01b, year==2017))
m18_con <- lm(lngap_just ~ mujer+edad+educacion+mov_subj_cwc+movsubj_mean ,data=subset(long01b, year==2018))
m19_con <- lm(lngap_just ~ mujer+edad+educacion+mov_subj_cwc+movsubj_mean ,data=subset(long01b, year==2019))

# mfx_con <- 
#   estimatr::lm_robust(lngap_just ~ year+mujer+edad+educacion+movsubj+lngap_perc,data=long01a, fixed_effects = ~idencuesta)

m16_cat <- lm(lngap_just ~ mujer+edad+educacion+sub_asc_cwc+sub_des_cwc+sub_asc_cgm+sub_des_cgm,data=subset(long01b, year==2016))
m17_cat <- lm(lngap_just ~ mujer+edad+educacion+sub_asc_cwc+sub_des_cwc+sub_asc_cgm+sub_des_cgm,data=subset(long01b, year==2017))
m18_cat <- lm(lngap_just ~ mujer+edad+educacion+sub_asc_cwc+sub_des_cwc+sub_asc_cgm+sub_des_cgm,data=subset(long01b, year==2018))
m19_cat <- lm(lngap_just ~ mujer+edad+educacion+sub_asc_cwc+sub_des_cwc+sub_asc_cgm+sub_des_cgm,data=subset(long01b, year==2019))

# mfx_cat <- 
#   estimatr::lm_robust(lngap_just ~ year+mujer+edad+educacion+mov_sub_c+lngap_perc,data=long01a, fixed_effects = ~idencuesta)

mfull_con <- 
  lme4::lmer(lngap_just ~ year + mujer + edad + educacion + mov_subj_cwc*lngap_perc+movsubj_mean +lngap_perc +  (1 | idencuesta),data=long01b)
mfull_cat  <- 
  lme4::lmer(lngap_just ~ year + mujer + edad + educacion + sub_asc_cwc*lngap_perc+sub_des_cwc*lngap_perc+sub_asc_cgm+sub_des_cgm + lngap_perc + (1  | idencuesta),data=long01b)

# mfull_con_rs <- 
#   lme4::lmer(lngap_just ~ year + mujer + edad + educacion + movsubj + lngap_perc + (1 + movsubj| idencuesta),data=long01b)
# mfull_cat_rs  <- 
#   lme4::lmer(lngap_just ~ year + mujer + edad + educacion + mov_sub_c + lngap_perc + (1 + mov_sub_c | idencuesta),data=long01b)

texreg::screenreg(l = list(m16_con,m17_con,m18_con,m19_con,
                           m16_cat,m17_cat,m18_cat,m19_cat
                           ))

texreg::screenreg(l = list(mfull_con, mfull_cat
                           ))
```
...
m07<- lmer(lngap_perc~year+year2+mujer+edad+educacion+ess+(1|idencuesta),data=long01b)
m08<- lmer(lngap_just~year+year2+mujer+edad+educacion+ess+lngap_perc + (1 | idencuesta),data=long01b)
m09<- lmer(lngap_just~year+year2+mujer+edad+educacion+ess+lngap_perc + (1 + lngap_perc | idencuesta),data=long01b)
